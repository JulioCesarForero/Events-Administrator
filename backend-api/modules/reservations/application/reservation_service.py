...
def create_reservation(
    ...
    *,
    ...
    if grp_check and grp_check.reservation_status == "CONFIRMED":
        raise ConflictError("Group already has an active reservation")
    ...
    if pay.status != "APPROVED":
        raise ConflictError("Payment must be approved before reserving")

    total_spots = sum(a.spots for a in allocations)
    if total_spots != pay.ticket_quantity:
        raise ValidationError("Total spots must match approved ticket quantity")
    ...
    if pol.status != "PUBLISHED" or terms.status != "PUBLISHED":
        raise ConflictError("Policy and terms must be published")
